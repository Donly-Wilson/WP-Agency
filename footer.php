<footer>
    <div class="title">
        <h1>Contact</h1>
    </div>
    <div class="contact">
        <div class="contact-info">
            <div class="email">
                <span>Email</span>
                <span>newera@agencey.com</span>
            </div>
            <div class="phone">
                <span>Phone</span>
                <span>+1 758 230 9214</span>
            </div>
            <div class="address">
                <span>Address</span>
                <span>Maigot bay, Kanel, 5th</span>
            </div>
            <div class="social-container">
                <a href="http://www.facebook.com/">
                    <i class="fab fa-facebook"></i>
                </a>
                <a href="http://www.dribbble.com/">
                    <i class="fab fa-dribbble"></i>
                </a>
                <a href="http://www.facebook.com/">
                    <i class="fab fa-twitter"></i>
                </a>
                <a href="http://www.facebook.com/">
                    <i class="fab fa-instagram"></i>
                </a>
            </div>
        </div>
        <div class="contact-form">
            <!-- contact 7 form -->
            <?php
            // echo do_shortcode(
            //     '[contact-form-7 id="12" title="Contact form 1"]'
            // );
            ?>
            <label for="name">Name<input type="text"></label>
            <label for="email">Email<input type="text"></label>
            <label for="phone">Phone<input type="text"></label>
            <label for="company">Company<input type="text"></label>
            <label for="website">Website<input type="text"></label>
            <label for="message">
                message
                <textarea name="" id="" cols="30" rows="10"></textarea>
            </label>
            <input type="submit" name="" id="">
        </div>
    </div>
    <h5>Donly Wilson</h5>
    <h6>Web Developer</h6>
</footer>